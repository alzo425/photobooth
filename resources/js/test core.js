...
const photoBooth = (function () {
    ...
    public.processPic = function (photoStyle, result) {
        ...
        $.ajax({
            ...
            success: (data) => {
                ...
                photoBooth.LEDcontroll("picture")
                if (data.error) {
    ...
    public.renderPic = function (filename) {
        ...
        // AZ if autoprint is activated the picture will immediately printed after taken.
        if(config.PrintAuto){
            public.AutoPrint(filename);
        }
        
        // AZ Aufruf Chroma Keying bei chroma aktiv -- not best solution
        if(config.chroma_keying){
            if (config.dev) {
                console.log('open chroma page');
            } 
            location = 'chromakeying.php?filename=' + encodeURI(filename);
        }
    ...
    public.openGallery = function () {
        ...
        photoBooth.LEDcontroll("gallery");
    ...
    //AZ LED Controll 
    public.LEDcontroll = function (modeIn) {
        const LED = {mode: modeIn};   
        jQuery.post('api/LEDcommunication.php', LED).done(function (result) {
                if (config.dev) {
                    console.log('LED mode ', result);
                }
                }).fail(function (xhr, status, result) {
                if (config.dev) {
                    console.log('Error LED', result);
                }   
            });   
    }
